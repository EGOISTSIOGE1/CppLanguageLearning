/*************************************************************************
	...
	> Created Time: Wed 15 Nov 2023 11:28:24 PM CST
 ************************************************************************/
...
#if DEBUG
void output(string& s, long long l, long long r) {
	for (int i = l; i < r; i++) {
		cout << s[i];
	}
	cout << endl;
...
long long calc(string& s, long long l, long long r) {
	...
	else { //分成两部分，前部分是l~pos，后部分是pos+1~r
		long long a = calc(s, l, pos); //左闭右开
		long long b = calc(s, pos + 1, r);
		switch (s[pos]) { //判断pos当前的符号来决定做什么操作
		case '+':return a + b;
		case '-':return a - b;
		case '*':return a * b;
		case '/':return a / b;
